...
__socket_reset (rpc_transport_t *this)
{
	...
	if (priv->sock % 2 == 0) {
        	...
        	event_unregister (this->ctx->event_pool2, priv->sock, priv->idx);
	}
#endif
        event_unregister (this->ctx->event_pool, priv->sock, priv->idx);
...
__socket_ioq_churn (rpc_transport_t *this)
{
		...
		if (priv->sock % 2 == 0) {
                	...
                	priv->idx = event_select_on (this->ctx->event_pool2,
                                             	priv->sock, priv->idx, -1, 0);
		}
#endif
               	priv->idx = event_select_on (this->ctx->event_pool,
			...
			if (priv->use_ssl && !priv->own_thread) {
				...
				if (new_priv->own_thread) {
					...
					if (new_sock % 2 == 0) {
						...
						new_priv->idx =
							event_register (ctx->event_pool2,
									new_sock,
									socket_event_handler,
									new_trans,
									1, 0);
...
socket_connect (rpc_transport_t *this, int port)
{
		...
		if (priv->own_thread) {
			...
			if (priv->sock % 2 == 0) {
				...
				priv->idx = event_register (ctx->event_pool2, priv->sock,
						    	socket_event_handler,
						    	this, 1, 1);
			}
#endif
	...
	char              a_byte = 'j';
        ...
        pthread_mutex_lock (&priv->lock);
        {
			...
			if (priv->sock % 2 == 0) {
                        	...
                        	priv->idx = event_select_on (ctx->event_pool2,
                                                     	priv->sock,
                                                     	priv->idx, -1, 1);
			}
#endif
...
socket_throttle (rpc_transport_t *this, gf_boolean_t onoff)
{
	...
	if (priv->sock % 2 == 0) {
        	...
        	priv->idx = event_select_on (this->ctx->event_pool2, priv->sock,
                                     	priv->idx, (int) !onoff, -1);
	}
#endif
       	priv->idx = event_select_on (this->ctx->event_pool, priv->sock,
